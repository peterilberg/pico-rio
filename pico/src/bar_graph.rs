...
pub async fn task(mut config: Config) {
    ...
    shift_value(&mut config, get_value(&selection, &measurements)).await;
...
async fn shift_value(config: &mut Config, value: u8) {
    ...
    config.latch.set_low();

    // Shift bits from lowest to highest.
    for _ in 0..8 {
        static LEVELS: [Level; 2] = [Level::Low, Level::High];
        config.data.set_level(LEVELS[(bits & 1) as usize]);
        bits >>= 1;

        config.clock.set_high();
        embassy_time::Timer::after_micros(1).await;
        config.clock.set_low();
    }

    config.latch.set_high();
}
